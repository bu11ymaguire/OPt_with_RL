...
**이 문서에서 참조하는 파일 경로는 첨부되지 않았다.** 코드와 행 단위 결과, 원고 소스는
아래 저장소로 공개할 예정이며 지금은 비어 있다. 세부를 직접 확인하고 싶으시면 말씀해
주시면 해당 파일을 보내드리겠다.

```text
https://github.com/bu11ymaguire/When_Does_Feedback_Help
```
...
프로토콜 이탈 `E1~E12` 는 `paper/draft.md §14` 에 전부 나열돼 있다.

`E12` 는 성격이 다르니 미리 밝힌다. **이 항목의 이전 판이 틀렸다.** 프로토콜이 규정한
동결 태그가 "만들어지지 않았다" 고 적었는데, 태그는 존재한다. 그 기록을 clone 환경에서
작성했고 태그를 원격에 올리지 않았기 때문에 생긴 오류다. 실제 이탈은 다른 것이다 —
태그가 동결 시점보다 4시간 뒤, confirmatory 실행 **이후**에 만들어졌고 동결 커밋이 아니라
Stage 2 종료 커밋을 가리킨다. 따라서 "held-out 실행 전에 config 가 고정됐다" 를 태그로
증명할 수 없고, 그 순서는 날짜가 붙은 결정 기록과 동결 커밋이 증명한다. 소급해서 태그를
옮기지 않았다.
