...
**라이브러리 버전이나 플랫폼이 다를 때의 비트 수준 일치는 주장하지 않는다.** 고정
시드를 써도 NumPy/SciPy 버전이나 플랫폼이 바뀌면 부동소수점 누적 순서가 달라져 극소한
차이가 생길 수 있다. byte-identical PDF 를 같은 고정 환경으로 한정한 것과 같은 이유다.
