...
**hostname 이 남은 곳을 구분한다.** 정리 이전 hostname 은 **private 소스 저장소의
이력**에 남아 있다. 공개 재현성 저장소는 독립 Git 이력을 갖는 allowlist export 이고
별칭 host 식별자만 담는다. 원고 `§15` 의 privacy note 가 이 구분을 명시해야 한다.
구분하지 않으면 독자가 공개 저장소 이력에도 hostname 이 남았다고 읽는다.
